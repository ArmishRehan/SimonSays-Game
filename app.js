...
function levelUp() {
    ...
    let randIdx = Math.floor(Math.random() * 4); // Fix range to 4
...
function checkAns(idx) {
    if (gameSeq[idx] === userSeq[idx]) {
        ...
        // Update high score if current level exceeds it
        if (level > highScore) {
            highScore = level - 1; // High score reflects the last completed level
        }

        // Show game over message with score and high score
